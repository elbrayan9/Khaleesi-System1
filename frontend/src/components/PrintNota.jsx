...
const PrintNota = React.forwardRef(
  ({ nota, datosNegocio, formatCurrency, clienteInfo }, ref) => {
    if (!nota) return null;
    ...
    return (
      <div ref={ref} className="hidden print:block" id="nota-imprimir-react">
        ...
        <hr />
        <p>
          <strong>NOTA DE {nota.tipo ? nota.tipo.toUpperCase() : ''}</strong>
        </p>
        ...
        {nota.ventaRelacionadaId && (
          <p>Ref. Venta: {nota.ventaRelacionadaId}</p>
        )}
        <hr />
        <p>
        ...
        <p className="total-final">
          MONTO TOTAL: ${formatCurrency(nota.monto)}
        </p>
        <hr />
