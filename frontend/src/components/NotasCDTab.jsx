// src/components/NotasCDTab.jsx
import React, { useState, useEffect, useRef } from 'react';
import { motion } from 'framer-motion';
import { PlusCircle, Trash2, ShoppingBag, XCircle, Eye, Printer } from 'lucide-react'; // Iconos añadidos
...
import {
  ...
  TableHeader,
  TableRow,
} from "@/components/ui/table"; // Ajusta esta ruta si es diferente

function NotasCDTab({
    notasCD = [],
    onAddNotaCD,
    onDeleteNotaCD,
    onViewDetailsNotaCD, // <-- NUEVA PROP
    onPrintNotaCD,       // <-- NUEVA PROP
    clientes = [],
    productos = [],
    formatCurrency,
    mostrarMensaje,
    ...
    const handleSelectProductoDev = (producto) => {
        ...
        setBusquedaProductoDev(producto ? producto.nombre : '');
    };

    const handleAgregarItemDevuelto = () => {
        if (!productoSeleccionadoParaDev || !isValidFirestoreIdForSelection(productoSeleccionadoParaDev.id)) {
            mostrarMensaje("Seleccione un producto válido (guardado) para la devolución.", "warning");
            return;
        }
        const cant = parseInt(cantidadDevolucion, 10);
        if (isNaN(cant) || cant <= 0) {
            mostrarMensaje("Ingrese una cantidad válida para la devolución.", "warning");
            return;
        }
        ...
        if (productoDevRef.current && typeof productoDevRef.current.clearInput === 'function') productoDevRef.current.clearInput();
    };

    const handleQuitarItemDevuelto = (index) => setItemsDevueltos(prev => prev.filter((_, i) => i !== index));

    const handleGenerarNotaClick = () => {
        ...
        if (isNaN(montoNota) || montoNota <= 0) { mostrarMensaje("Ingrese un monto válido para la nota.", 'warning'); return; }
        if (implicaDevolucion && tipoNota === 'credito' && itemsDevueltos.length === 0) {
            mostrarMensaje("Si la nota de crédito implica devolución, debe agregar productos.", "warning"); return;
        }
        if (implicaDevolucion && tipoNota === 'credito' && itemsDevueltos.some(item => !isValidFirestoreIdForSelection(item.id))) {
            mostrarMensaje("Lista de productos devueltos contiene ítems con IDs inválidos.", "error"); return;
        }
        ...
        onAddNotaCD(notaDataParaApp);
        setTipoNota('credito'); setVentaRelacionadaId(''); setClienteId('');
        setMotivo(''); setMonto(''); setImplicaDevolucion(false); setItemsDevueltos([]);
        setProductoSeleccionadoParaDev(null); setCantidadDevolucion(1); setBusquedaProductoDev('');
        if (productoDevRef.current && typeof productoDevRef.current.clearInput === 'function') productoDevRef.current.clearInput();
    };

    // Helper para deshabilitar acciones si el ID de la nota no es válido (ej. un ID local temporal)
    ...
    return (
        <div id="notas_cd">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                {/* Formulario para generar nueva nota */}
                <div className="bg-zinc-800 p-4 sm:p-5 rounded-lg shadow-md">
                    ...
                    <div className="space-y-3">
                        {/* Tipo de Nota */}
                        <div>
                            <label htmlFor="nota-tipo-form" className="block text-sm font-medium text-zinc-300 mb-1">Tipo de Nota:</label>
                            <select id="nota-tipo-form" value={tipoNota} onChange={(e) => setTipoNota(e.target.value)} className="w-full p-2 border border-zinc-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-zinc-700 text-zinc-100">
                                <option value="credito">Nota de Crédito</option>
                                <option value="debito">Nota de Débito</option>
                            </select>
                        </div>
                        {/* Venta Relacionada */}
                        <div><label htmlFor="nota-venta-rel-form" className="block text-sm font-medium text-zinc-300 mb-1">Venta Relacionada (ID - Opcional):</label><input type="text" id="nota-venta-rel-form" value={ventaRelacionadaId} onChange={(e) => setVentaRelacionadaId(e.target.value)} placeholder="ID de la Venta Original" className="w-full p-2 border border-zinc-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-zinc-700 text-zinc-100 placeholder-zinc-400"/></div>
                        {/* Cliente */}
                        <div>
                            <label htmlFor="nota-cliente-form" className="block text-sm font-medium text-zinc-300 mb-1">Cliente:</label>
                            <select id="nota-cliente-form" value={clienteId} onChange={(e) => setClienteId(e.target.value)} className="w-full p-2 border border-zinc-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-zinc-700 text-zinc-100" required>
                                <option value="">Seleccione un Cliente...</option>
                                {clientesValidos.map(c => (<option key={c.id} value={c.id}>{c.nombre} ({c.cuit || 'S/N CUIT'})</option>))}
                            </select>
                        </div>
                        {/* Motivo */}
                        <div><label htmlFor="nota-motivo-form" className="block text-sm font-medium text-zinc-300 mb-1">Motivo:</label><textarea id="nota-motivo-form" value={motivo} onChange={(e) => setMotivo(e.target.value)} rows="2" placeholder="Ej: Devolución, ajuste de precio..." className="w-full p-2 border border-zinc-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-zinc-700 text-zinc-100 placeholder-zinc-400" required></textarea></div>
                        {/* Monto */}
                        <div><label htmlFor="nota-monto-form" className="block text-sm font-medium text-zinc-300 mb-1">Monto Total de la Nota ($):</label><input type="number" id="nota-monto-form" value={monto} onChange={(e) => setMonto(e.target.value)} step="0.01" min="0.01" placeholder="Monto total de la nota" className="w-full p-2 border border-zinc-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-zinc-700 text-zinc-100" required /></div>

                        {/* Checkbox para Devolución de Productos */}
                        {tipoNota === 'credito' && (
                        ...
                        {/* Sección para Agregar Productos Devueltos */}
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 items-end">
                                    <div className="sm:col-span-2">
                                        ...
                                        <SearchBar
                                            ...
                                            initialValue={busquedaProductoDev}
                                        />
                                    </div>
                                    <div>
                                        <label htmlFor="cantidad-devolucion" className="block text-xs font-medium text-zinc-400 mb-1">Cantidad:</label>
                                        <input type="number" id="cantidad-devolucion" value={cantidadDevolucion} onChange={(e) => setCantidadDevolucion(e.target.value)} min="1" className="w-full p-2 border border-zinc-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-zinc-700 text-zinc-100"/>
                                    </div>
                                </div>
                                <button type="button" onClick={handleAgregarItemDevuelto} disabled={!productoSeleccionadoParaDev || !cantidadDevolucion || Number(cantidadDevolucion) <=0} className={`w-full text-sm font-medium py-1.5 px-3 rounded-md transition duration-150 ease-in-out inline-flex items-center justify-center ${(!productoSeleccionadoParaDev || !cantidadDevolucion || Number(cantidadDevolucion) <=0) ? 'bg-zinc-500 text-zinc-400 cursor-not-allowed' : 'bg-sky-600 hover:bg-sky-700 text-white'}`}>
                                    ...
                                    <div className="mt-2 max-h-32 overflow-y-auto pr-1 space-y-1">
                                        ...
                                        {itemsDevueltos.map((item, index) => (
                                            <div key={`${item.id}-${index}`} className="flex justify-between items-center text-xs bg-zinc-700/70 p-1.5 rounded">
                                                ...
                                                <button onClick={() => handleQuitarItemDevuelto(index)} className="text-red-400 hover:text-red-300 p-0.5" title="Quitar">
                                                    <XCircle size={16}/>
                                                </button>
                                            </div>
                                        ))}
                        ...
                        <div className="text-right mt-4">
                            <motion.button onClick={handleGenerarNotaClick} className="w-full sm:w-auto bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-3 rounded-md transition duration-150 ease-in-out inline-flex items-center" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                                <PlusCircle className="h-4 w-4 mr-2"/>Generar Nota
                            </motion.button>
                        </div>
                    </div>
                </div>

                {/* Tabla de Notas Generadas */}
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="hover:bg-transparent border-b-zinc-700">
                                    ...
                                    <TableHead className={`${thClasses} text-right`}>Monto</TableHead>
                                    <TableHead className={`${thClasses} text-center`}>Acciones</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {notasCD.slice().sort((a, b) => new Date(b.timestamp || 0) - new Date(a.timestamp || 0)).map(n => (
                                    <TableRow key={n.id || `nota_fb_${Date.now()}${Math.random()}`} className="hover:bg-zinc-700/30">
                                        ...
                                        <TableCell className={`${tdClasses} text-center whitespace-nowrap`}>
                                            ...
                                            {typeof onViewDetailsNotaCD === 'function' && (
                                                <motion.button
                                                    ...
                                                    className="text-purple-400 hover:text-purple-300 mr-2 p-1 rounded"
                                            ...
                                            {typeof onPrintNotaCD === 'function' && (
                                                <motion.button
                                                    ...
                                                    className="text-blue-400 hover:text-blue-300 mr-2 p-1 rounded"
                                                    title="Imprimir Nota"
                                                    whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                                                    disabled={isActionDisabled(n)}
                                                >
                                                    <Printer className="h-4 w-4 inline-block"/>
                                                </motion.button>
                                            )}
                                            <motion.button
                                                onClick={() => onDeleteNotaCD(n.id)}
