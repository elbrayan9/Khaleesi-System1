...
const CajaGeneral = () => {
    ...
    const totalCajaGeneral = useMemo(() => {
        ...
        const totalIngresos = ingresosManuales.reduce((total, ingreso) => total + ingreso.monto, 0);
    ...
    return (
        <div className="bg-gradient-to-r from-cyan-500 to-blue-600 p-6 rounded-lg shadow-lg text-white">
            <div className="flex items-center gap-4">
                <PiggyBank size={40} />
                <div>
                    <p className="text-lg font-semibold">Total en Caja General</p>
                    <p className="text-sm opacity-80">Saldo acumulado de todos los movimientos</p>
                </div>
            </div>
            <p className="text-4xl font-extrabold text-right mt-4">
